...
import { ProfileTab, SecurityTab, NotificationsTab, ScanPreferencesTab, GitIntegrationTab, AppearanceTab } from './tabs'
import Toast, { useToast } from './components/Toast'

const TABS = [
  { id: 'profile', label: 'Profile', icon: User, desc: 'Personal details' },
  { id: 'git-integration', label: 'Repository & Git', icon: GitBranch, desc: 'Main repo configuration' },
              ...
              {TABS.map((tab) => (
                ...
                >
                  <tab.icon className={cn(
                    ...
                    activeTab === tab.id ? 'text-emerald-400' : 'text-steel-500 group-hover:text-steel-300'
                  )} />
                  <div className="flex-1 min-w-0">
                    <span className={cn(
                      'block text-sm font-medium truncate',
                      activeTab === tab.id ? 'text-emerald-400' : 'text-steel-300 group-hover:text-steel-100'
                    )}>
                      {tab.label}
                    </span>
